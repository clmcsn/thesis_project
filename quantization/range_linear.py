...
#added library
from common.nnTools import getSparsity
from common.mask_util import mask_param, MaskType, MaskTable, get_max_masked_val
...
class RangeLinearQuantWrapper(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        try:
            with open(path_conf["validity_dump_file"],"r") as inpointer:
                if isinstance(self.wrapped_module, (nn.Conv2d, nn.Conv3d, nn.Linear)):
                    torch.save(accum,path_conf["dumps"]+path_conf["ref_dumps_file"].format(self.distiller_name))
...
class RangeLinearQuantParamLayerWrapper(RangeLinearQuantWrapper):
    ...
    def __init__(self, wrapped_module, num_bits_acts, num_bits_params, num_bits_accum=32,
                 mode=LinearQuantMode.SYMMETRIC, mask_table=None, clip_acts=ClipMode.NONE, per_channel_wts=False, activation_stats=None,
        ...
        w_scale, w_zero_point = _get_quant_params_from_tensor(wrapped_module.weight,
                                                              ...
                                                              correctRange=mask_table.Table[wrapped_module.distiller_name].correctRange,
                                                              mask=mask_table.Table[wrapped_module.distiller_name].mask)
        ...
        if mask_table:
            wrapped_module.weight.data = mask_param( wrapped_module.weight.data
                        ,mask_table.Table[wrapped_module.distiller_name].mask
    ...
    def quantized_forward(self, input_q):
        ...
        if self.preset_act_stats:
            ...
            if self.has_bias:
                ...
                try:
                    with open(path_conf["validity_dump_file"],"r") as inpointer:
                        with open (path_conf["dumps"]+path_conf["sf_dump_file"],"a") as out_p:
                            out_p.write("{}\n".format(float(bias_requant_scale.data)))
                except FileNotFoundError:
                    pass
...
class PostTrainLinearQuantizer(Quantizer):
    ...
    def __init__(self, model, bits_activations=8, bits_parameters=8, bits_accum=32,
                 overrides=None, mode=LinearQuantMode.SYMMETRIC, mask_table=None, clip_acts=ClipMode.NONE,
        ...
        def replace_param_layer(module, name, qbits_map, per_channel_wts=per_channel_wts,
            ...
            return RangeLinearQuantParamLayerWrapper(module, qbits.acts, qbits.wts,
                                                     num_bits_accum=self.bits_accum, mode=mode, mask_table=mask_table, clip_acts=clip_acts,
                                                     per_channel_wts=per_channel_wts,
                                                     activation_stats=activation_stats,
                                                     clip_n_stds=clip_n_stds, clip_half_range=clip_half_range,
                                                     scale_approx_mult_bits=scale_approx_mult_bits,
...
class QuantAwareTrainRangeLinearQuantizer(Quantizer):
    def __init__(self, model, optimizer=None, bits_activations=32, bits_weights=32, bits_bias=32,
                 overrides=None, mode=LinearQuantMode.SYMMETRIC, mask_table=None, ema_decay=0.999, per_channel_wts=False,
        ...
        def linear_quantize_param(param_fp, param_meta):
            ...
            with torch.no_grad():
                scale, zero_point = _get_quant_params_from_tensor(param_fp, param_meta.num_bits, mode,
                                                                  per_channel=perch, correctRange=mask_table.correctRange,
                                                                  mask=mask_table.mask)
            setattr(m, param_meta.q_attr_name + '_scale', scale)
            setattr(m, param_meta.q_attr_name + '_zero_point', zero_point)
            out = LinearQuantizeSTE.apply(param_fp, scale, zero_point, True, False, self.default_qbits.wts, mode != LinearQuantMode.ASYMMETRIC_UNSIGNED, mask_table.mask_type, mask_table.mask)
            return out
