    ...
    for quant_mode in s.quant_mode_list:
        for mask_mode in s.mask_mode_list:
            for i in range(1,s.stop_string): #all possible mask
                ...
                ref_quantized = PostTrainLinearQuantizer( deepcopy(network), bits_activations=aw_bits, bits_parameters=aw_bits, bits_accum=acc_bits,
                ...
                balanceNetwork(ref_quantized.model,
                                ...
                                batch_size=230,
                                device="cpu")
                quantized_child1.model.to(device)
                test_preds = get_all_preds(quantizer.model, data_loader,device=device)
                preds_correct = test_preds.argmax(dim=1).eq(torch.LongTensor(train_set.targets).to(device)).sum().item()
                log_pointer.write(s.rep_string.format(quant_mode,mask_mode,mask))
