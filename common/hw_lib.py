...
class multiplier:
    ...
    def newCsaRow(self,tag,start,lenght):
        if ((start+lenght)>self.parallelism*2):
            print("Csa chain too long. Please check your design tag:{} start:{} lenght:{} string:{}".format(tag,start,lenght,self.weightValidationBits[::-1]))
            exit()
    ...
    def genCsaMultiplier(self):
        ...
        with open(fileName,"w") as fout_pointer, open(s.templatePath+fileName,"r") as fin_pointer, open(s.csa_outPath,"r") as auto_pointer:
            fout_pointer.write("//CSA autogenerated with string {}\n//Date:{date}".format(self.weightValidationBits[::-1],date=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        ...
        self.pp_indexes=[0 for i in self.pp_indexes]
        self.sum_inIndex=[0 for i in self.sum_inIndex]
        self.carry_inIndex=[0 for i in self.carry_inIndex]
        self.prod_index=0
        self.fh_count=0
